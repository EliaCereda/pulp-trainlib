...
//Connecting linear
  ...
  // Layer 8
  l8_args.input = &input_blob;
  l8_args.output = &output_blob;
  // Layer 9
...
void forward()
{
	...
	copy_struct_param((unsigned int) &l0_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	copy_struct_param((unsigned int) &l2_args, (unsigned int) &MaxPool_args, sizeof(MaxPool_args));
	...
	load_coeff(&layer3_wgt, 1);
	copy_struct_param((unsigned int) &l3_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	copy_struct_param((unsigned int) &l5_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	load_coeff(&layer7_wgt, 1);
	copy_struct_param((unsigned int) &l7_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	get_output_dim(&layer7_out);
	pulp_conv2d_fp32_fw_cl(&conv2d_args);
	store_output(&layer7_out, 1);

	reset_dim();
	load_input(&layer8_in, 1);
	get_output_dim(&layer8_out);
	pulp_relu_fp32_fw_cl(&act_args);
	store_output(&layer8_out, 1);

	reset_dim();
	load_input(&layer9_in, 1);
	load_coeff(&layer9_wgt, 1);
	copy_struct_param((unsigned int) &l9_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	copy_struct_param((unsigned int) &l11_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	copy_struct_param((unsigned int) &l13_args, (unsigned int) &conv2d_args, sizeof(conv2d_args));
	...
	copy_struct_param((unsigned int) &l15_args, (unsigned int) &linear_args, sizeof(linear_args));
...
void backward()
{
	...
	pulp_conv2d_fp32_bw_cl(&conv2d_args);
	...
	reset_dim();
	load_input(&layer8_in, 1);
	load_output(&layer8_out, 2);
	pulp_relu_fp32_bw_cl(&act_args);
	store_input(&layer8_in, 0);
...
void compute_loss()
{
  ...
  pi_cl_dma_cmd((uint32_t) (LABEL), (uint32_t) (output_blob.diff), 4*OUT_SIZE, PI_CL_DMA_DIR_EXT2LOC , cmd_load);
  pi_cl_dma_cmd_wait(cmd_load);
  pulp_MSELoss(&loss_args);
  store_output(&layer15_out, 2);
}
